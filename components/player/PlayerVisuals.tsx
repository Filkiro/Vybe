...
// ---------------------------------------------------------------
export function BotaoCurtir({ curtido, totalCurtidas, onPress }: { curtido: boolean; totalCurtidas?: number; onPress: () => void }) {
  return (
    <Pressable
      onPress={onPress}
      className="flex-row items-center gap-2 h-10 px-3 rounded-full bg-white/10 justify-center"
    >
      <Heart color={curtido ? "#EF4444" : "#FFFFFF"} size={20} fill={curtido ? "#EF4444" : "transparent"} />
      {totalCurtidas !== undefined && (
        <Text className="text-white font-semibold text-sm">{totalCurtidas}</Text>
      )}
    </Pressable>
  );
}
