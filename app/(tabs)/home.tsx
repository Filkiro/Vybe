    ...
    <View style={{ flex: 1, backgroundColor: '#0B101E' }}>
      {carregando ? (
        ...
        <FlatList
          ...
          numColumns={ehDesktop ? 3 : 1}
          columnWrapperStyle={ehDesktop ? { gap: 16, paddingHorizontal: 20, maxWidth: 1200, alignSelf: 'center', width: '100%', marginBottom: 16 } : undefined}
          ...
          renderItem={({ item }) => (
            <View 
              className={ehDesktop ? "" : "w-full max-w-[1200px] self-center px-5"}
              style={ehDesktop ? { width: '32%' } : undefined}
            >
              <Pressable
                onPress={() => {
                  requireAuth(() => {
              ...
              <View className="flex-row items-center justify-between px-4 py-3">
                <View className="flex-row items-center gap-4 flex-1">
                  {item.capa_url ? (
                    <Image source={{ uri: item.capa_url }} className="w-14 h-14 rounded-lg" />
                  ) : (
                ...
                <View className="w-10 h-10 rounded-full bg-white/10 items-center justify-center border border-white/20">
                  <Play color="#8B95A8" size={16} fill="#8B95A8" style={{ marginLeft: 2 }} />
                </View>
              </View>
            </Pressable>
            </View>
